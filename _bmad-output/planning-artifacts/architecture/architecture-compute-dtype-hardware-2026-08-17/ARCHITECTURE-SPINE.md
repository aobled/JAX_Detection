...
status: final
created: '2026-08-17'
updated: '2026-08-17 (AD-4 amendé : BatchNorm/LayerNorm suivent désormais compute_dtype)'
binds: ['CAP-1', 'CAP-2']
...
flowchart LR
    ...
    F --> G["SophisticatedCNN32Plus /\nSophisticatedCNN128Lite / 128Plus /\nKepler1DConvNet\n(AD-4, AD-6)"]
...
### AD-4 — compute_dtype s'applique aux couches matmul lourdes ET à la normalisation, jamais à Embed [AMENDÉ 2026-08-17]

- **Binds:** toute classe (top-level ou sous-module partagé) gagnant `compute_dtype` — aujourd'hui `SophisticatedCNN32Plus`, `SophisticatedCNN128Lite`, `SophisticatedCNN128Plus`, `Kepler1DConvNet`, `SeparableConv`, `SEBlock`, `SpatialAttention` ; s'applique automatiquement à tout futur adoptant sans réécrire cette règle
- **Prevents:** un traitement spécial ad hoc et incohérent de la normalisation d'un modèle à l'autre ; une propagation partielle silencieuse (un site d'appel oublié dans un sous-module produit un mix bfloat16/float32 sans erreur ni log) ; une chaîne de précision réduite interrompue à chaque bloc de normalisation, qui viderait une partie du gain de vitesse visé
- **Rule:** `dtype=self.compute_dtype` s'applique à **tout** appel `nn.Conv`/`nn.Dense`/`nn.BatchNorm`/`nn.LayerNorm`, **où qu'il apparaisse** — y compris à l'intérieur des sous-modules partagés (AD-5) — jamais à `nn.Embed` (gather non compute-bound, aucun bénéfice, source unique : docstring `ChessMoveTokenTransformer`, `model_library.py:1107-1123`).
  **Historique** : la version originale de cette règle (2026-08-17, groupe CIFAR10/FIGHTERJET) excluait aussi `BatchNorm`/`LayerNorm`, au motif que seules les couches matmul lourdes bénéficient d'un calcul en précision réduite — vrai en isolation, mais incomplet : la revue Blind Hunter du groupe 1 du rollout (`spec-compute-dtype-rollout-classification.md`) a montré empiriquement (`capture_intermediates` sur `SophisticatedCNN32Plus`/`128Plus` réels) que ne PAS passer `dtype=` à `BatchNorm`/`LayerNorm` fait ressortir leur sortie en `float32` systématiquement, quel que soit le dtype d'entrée — la chaîne `compute_dtype` se réinitialise donc à chaque bloc au lieu de tenir en continu. Vérifié sûr avant amendement : `force_float32_reductions=True` (défaut Flax) protège la réduction interne de `BatchNorm`/`LayerNorm` indépendamment de `dtype=` — confirmé empiriquement pour les deux (`batch_stats`/`params` restent `float32` même avec `dtype=jnp.bfloat16` explicite). AD-6 inchangé (poids/stats toujours `float32`).
...
- **Rollout aux 7 autres modèles de `MODELS`** (`aircraft_detector_unet`/`centernet`/`centernet_lite`, `chess_cnn_attention_policy_value`/`legal_moves`, `chess_token_candidate_model`, `chess_token_one_move_model`) — non touchés par cette spine (AD-3 garantit qu'ils ne cassent pas). *(Mis à jour 2026-08-17 : `sophisticated_cnn_128_plus` et `kepler_1d_cnn` sont sortis de cette liste, adoptés par `spec-compute-dtype-rollout-classification.md` — groupe 1 du rollout livré ; 5 modèles adoptés au total avec `chess_move_token_transformer`.)* Décision à reprendre modèle par modèle, séquencement voulu par Aymeric (groupe 2 : `aircraft_detector_centernet_lite`, groupe 3 : `centernet`+`unet` ensemble — `deferred-work.md`). Pour les modèles `aircraft_detector_*`, qui composent les mêmes sous-modules partagés (`SeparableConv`/`SEBlock`/`SpatialAttention`) : AD-5 s'applique intégralement, un site d'appel oublié produirait un mix silencieux — le test requis par AD-3 est la garde-fou. **Important pour ce rollout futur** : l'amendement AD-4 (BatchNorm/LayerNorm suivent désormais `compute_dtype`) doit être appliqué dès l'implémentation initiale de ces modèles, pas retrofité après coup comme ça a été le cas pour le groupe 1.
...
- ~~**Mise à jour de `SPEC-compute-dtype-hardware`**~~ — fait (`bmad-spec`, 2026-08-17, révision OQ1 documentée ; ré-amendé le même jour pour AD-4 BatchNorm/LayerNorm).
...
- **Validation matérielle réelle de l'amendement AD-4 (BatchNorm/LayerNorm)** — contrairement au reste de cette spine (CIFAR10/FIGHTERJET validés réel TPU v5, 0.9459 val accuracy), l'amendement AD-4 lui-même n'a été validé QUE par tests unitaires synthétiques (`pytest`, entrées à zéro). Aymeric doit relancer FIGHTERJET_CLASSIFICATION sur Colab TPU pour confirmer que le comportement numérique réel (pas seulement le dtype) reste sain sous ce changement, avant de considérer l'amendement pleinement validé.
