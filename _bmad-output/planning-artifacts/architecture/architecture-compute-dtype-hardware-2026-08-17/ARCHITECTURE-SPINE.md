...
- ~~**Rollout aux modèles non-échecs restants de `MODELS`**~~ — **TERMINÉ (2026-08-17).** Les 6 modèles non-échecs de `MODELS` ont tous `compute_dtype` : `sophisticated_cnn_32_plus`/`128_lite`/`128_plus`, `kepler_1d_cnn` (`spec-compute-dtype-rollout-classification.md`), `aircraft_detector_unet`/`centernet` (`spec-compute-dtype-detection.md`). `aircraft_detector_centernet_lite` s'est révélé être du code mort (aucune config active, testé puis abandonné le 2026-07-26) — supprimé plutôt qu'adapté. Seuls les 5 modèles échecs (`chess_cnn_attention_policy_value`/`legal_moves`, `chess_token_candidate_model`, `chess_token_one_move_model` — `chess_move_token_transformer` déjà fait) restent hors périmètre, explicitement exclus par Aymeric ("moins importants, à revoir plus tard").
