...
- `compute_dtype` s'applique aux couches matmul lourdes (`nn.Conv`/`nn.Dense`) ET à la normalisation (`nn.BatchNorm`/`nn.LayerNorm`) — jamais à `nn.Embed`. *(Amendé 2026-08-17 : la version initiale excluait aussi BatchNorm/LayerNorm ; la revue du rollout a montré que cette exclusion réinitialise la chaîne de précision réduite à `float32` à chaque bloc — vérifié sûr avant amendement, `force_float32_reductions` protège la réduction interne indépendamment de `dtype=`. Détail dans le companion `ARCHITECTURE-SPINE.md`, AD-4.)*
